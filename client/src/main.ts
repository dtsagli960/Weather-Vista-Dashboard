...
const renderCurrentWeather = (currentWeather: {
  ...
  console.log("renderCurrentWeather", currentWeather);
  if (!currentWeather) {
    console.error("Error: currentWeather is undefined");
    return;
  }
  const { name, date, icon, iconDescription, temp, wind, humidity } = currentWeather;

  const heading = document.getElementById("search-title") as HTMLElement | null;
  console.log(heading);
  const weatherIcon = document.getElementById("weather-img") as HTMLImageElement | null;
  console.log(weatherIcon);
  const tempEl = document.getElementById("temp") as HTMLElement | null;
  console.log(tempEl);
  const windEl = document.getElementById("wind") as HTMLElement | null;
  console.log(windEl);
  const humidityEl = document.getElementById("humidity") as HTMLElement | null;
  console.log(humidityEl);
  const todayContainer = document.getElementById("today") as HTMLElement | null;
...
const renderForecast = (forecast: any): void => {
  ...
  heading.textContent = '1-Day Forecast:';
